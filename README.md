...
- `r.InstantRdv.Vsp.VisProbe` で ActiveProbe デバッグ可視化（0:OFF, 1:生存状態, 2:インデックスhash, 3:age, 4:cascade, 5:OctMap radiance, 6:OctMap sky visibility, 7:SH radiance, 8:SH sky visibility, 9:BBV埋まり判定）
- `r.InstantRdv.Vsp.VisIvProbe` で IrradianceVolume プローブ可視化（0:OFF, 1:SH radiance, 2:SH sky visibility）
- `r.InstantRdv.Vsp.TraceUseProbeOffset` で VSP ray trace の始点に Probe offset を適用するか切り替え可能
- `r.InstantRdv.Vsp.VisProbeUseOffset` で ActiveProbe デバッグ球の表示位置に Probe offset を適用するか切り替え可能
- ActiveProbeListは他のVSP counter bufferと異なり、word 0/1を世代交代counter、word 2以降をProbe indexとして使用する。これはGPUフレーム重複時のreset/append競合を避けるための専用レイアウトである
- VSP は InstantRDV 参照実装の ActiveProbe lifecycle（visible surface収集、probe pool/free stack、ray request/trace/resolve、OctMap、SH更新、IrradianceVolume伝播）をUE RDG passへ移植中
